...
class PRM:
    def __init__(self, pr, is_colliding):
        ...
        self._radius = 0.5
        self._k = 15
        self.maximum_number_of_specified_nodes = int(150000)
    ...
    def _is_seg_valid(self, q0, q1):
        """
        To check if the edge between q0 and q1 is collision free by interpolating the segment
        """
    ...
    def plan(self, q_start, q_target, constr=None, args=None):
        ...
        if args.reuse_graph:
            ...
            print("PRM: To reuse the graph.")
        else:
            graph = GraphPRM(len(q_start), capacity=180000)
            s = time()
            self.preprocess(graph, constr)
            print('PRM: We construct the graph in {:.2f}s'.format(time() - s))

            with open(graph_name, 'wb') as f:
                pickle.dump(graph, f, -1)
                print('PRM: The graph has been saved!')

        s = time()
        graph.start_id = graph.add_npde(q_start)
        neighbor_node_ids = graph.list_neighbors_within_distances(q_start, 1.5)  # 1.5 for map3
        print('PRM: Found neighbor {} with q_start'.format(len(neighbor_node_ids)))
        for neighbor_id in neighbor_node_ids:
            q_neighbor = graph.return_corresponding_point(neighbor_id)
            if self._is_seg_valid(q_start, q_neighbor):
                graph.add_edge(graph.start_id, neighbor_id)

        graph.target_id = graph.add_npde(q_target)
        neighbor_node_ids = graph.list_neighbors_within_distances(q_target, 1.5)  # 1.5 for map3
        print('PRM: Found neighbor {} with q_target'.format(len(neighbor_node_ids)))
        for neighbor_id in neighbor_node_ids:
            q_neighbor = graph.return_corresponding_point(neighbor_id)
            if self._is_seg_valid(q_target, q_neighbor):
                graph.add_edge(graph.target_id, neighbor_id)

        print('PRM: The number of nodes connected with the source node are: {}'.format(len(graph.find_out_parent(graph.start_id))))
        print('PRM: The number of nodes connected with the target node are: {}'.format(len(graph.find_out_parent(graph.target_id))))
