...
class OBPRM:
    ...
    def sample_valid_joints(self):
        ...
        """
        q = np.random.random(self._fr.num_dof) * (self._fr.upper_joint_limits - self._fr.lower_joint_limits) + self._fr.lower_joint_limits
        return q

    def sample_near_joints(self, q):
        q_near = q
        for i in range(len(q)):
            lower = max(self._fr.lower_joint_limits[i], q[i] - self._joint_size)
            upper = min(self._fr.upper_joint_limits[i], q[i] + self._joint_size)
    ...
    def project_to_constraint(self, q0, constraint):
        ...
        while err > self._constraint_th:
            J = self._fr.analytical_jacobian(q_proj)
    ...
    def plan_path(self, q_start, q_target, constraint=None, args=None):
        if args.map3:
